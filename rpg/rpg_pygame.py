...
def show_status_console():
    """Print status only when player moves or takes item"""
    global has_map
    ...
    # Only show map if player has the map
    if "map" in inventory:
        if not has_map:
            has_map = True
...
while True:
    ...
    # Win condition: all main items collected
